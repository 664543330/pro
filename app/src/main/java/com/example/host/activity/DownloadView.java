...
public class DownloadView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray typeArray = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.DownloadView, 0, 0);
        mRadius = (int)typeArray.getDimension(R.styleable.DownloadView_radius, 100);
        mNotBeginImg = ((BitmapDrawable)typeArray.getDrawable(R.styleable.DownloadView_notBeginImg)).getBitmap();
        mPausedImg = ((BitmapDrawable)typeArray.getDrawable(R.styleable.DownloadView_pausedImg)).getBitmap();
        mWatiImg = ((BitmapDrawable)typeArray.getDrawable(R.styleable.DownloadView_waitImg)).getBitmap();
        finishedImg = ((BitmapDrawable)typeArray.getDrawable(R.styleable.DownloadView_finishedImg)).getBitmap();
        ...
        mStrokeWidth = (int)typeArray.getDimension(R.styleable.DownloadView_strokeWidth, 2);

//		mRadius = Math.max(mNotBeginImg.getWidth()/2, mNotBeginImg.getHeight()/2) + mStrokeWidth;
        mCircleColor = typeArray.getColor(R.styleable.DownloadView_circleColor, 0xFFFFFFFF);
        mRingColor = typeArray.getColor(R.styleable.DownloadView_ringColor, 0xFFFFFFFF);
    }
    ...
    private void drawDownloadingView(Canvas canvas) {
        ...
        //绘制中间百分比文字
        String percentTxt = String.valueOf((int) (mProgress*100)/mTotalProgress);
        //计算文字垂直居中的baseline
